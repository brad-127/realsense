...
while True:
    ...
    # V の値を 0.5 に変更（0-255の範囲で0-255に戻す）
    resized_hsv[..., 2] = 127  # 0.5 * 255 = 127
    # HSV を BGR に戻す
    # print(f"\r{resized_hsv}", end="")
    modified_bgr = cv2.cvtColor(resized_hsv, cv2.COLOR_HSV2BGR)
    resized_rgb = cv2.cvtColor(modified_bgr, cv2.COLOR_BGR2RGB)
    ...
    # Show the RGBD image
    cv2.imshow("RBG Image", modified_bgr)
    ...
    cv2.imwrite(rgb_filename, modified_bgr)
